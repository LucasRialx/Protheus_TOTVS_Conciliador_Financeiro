...
def compare_item_values(df_A, df_B, item_column_A, value_column_A, item_column_B, value_column_B):
    ...
    df_merged = pd.merge(df_A, df_B, left_on=item_column_A, right_on=item_column_B, how='outer', suffixes=('_A', '_B'))
    ...
    df_differences = df_differences.rename(columns={
        item_column_A: 'CODIGO FORNECEDOR C.',
        value_column_A: 'VALOR CONTABIL',
        item_column_B: 'CODIGO FORNECEDOR F.',
...
planilha_A = 'DATABASE_BALANCETE_ADIANTAMENTO_FORNECEDOR.xlsx'
planilha_B = 'ADT_FORN_TITULOS_A_PAGAR.xlsx'
...
df_B = pd.read_excel(os.path.join(input_folder_B, planilha_B), sheet_name='Sheet1', dtype={'BC_concatenado': str})
...
item_column_B = 'BC_concatenado'  # Coluna de itens na Planilha B
value_column_B = 'DF_soma'  # Coluna de valores na Planilha B
...
result_filename = 'RESULTADO_ADT_FORNECEDOR.xlsx'
